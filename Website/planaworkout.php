	...
	<form id= "sub" name = "plan" action="submit.php" method = "POST">
		...
		<div id ="main" class = "contentbox">
			...
			<div id="adder" class="buttonBox">
				<p>Select Custom To Create New Categories And Workouts</p>
		...
		<input type="hidden" id="obj" class ="obj" name="obj" value="">
	</form>
...
<script>
	...
	$(document).on('click','.deletebtn', function(e){
		...
		//Delete Comfirmation
		ans = confirm("Removing this plan will result in the removal of all plans history. Are you sure you want to delete '"+confirmPlan+"' ?");
		if (ans == true) {
	...
	$(document).on('click','.editPlanbtn',function(e){
		...
		for (z=0; z<userWorkoutsArr.length;z++) {
			if (userWorkoutsArr[z]["plan_ID"] == planID) {
				...
				createListElement(z,userWorkoutsArr[z]["catagory_name"],userWorkoutsArr[z]["workout_name"],userWorkoutsArr[z]["unit"],amount,userWorkoutsArr[z]["weight"],userWorkoutsArr[z]["workout_ID"]);    
			};
		};
	});
	...
	function createListElement(count,catName,workoutName,measurement,amount,weight,workout_ID){

			var temptableID = "table_"+count;
			var tempdelID = "del_"+count;
			var templiID = "li_"+count;
			var tempCopyID = "copy_"+count;
			var tempHidID = "input_"+count;
	...
	$("#Add").click(function (e) {
		...
		if (selectedCat == "Custom") {
			...
			if ($("#timereps").val() == ""){
				...
				count++;
				createListElement(count,getCatagoryName(),getWorkoutName(),measurement,$("#timereps").val(),weight,getWorkoutID());
			};
		};
	});
</script>
